...
function sendGuideEmail(string $email, bool $marketingConsent = false): array {
    $key = getenv('BREVO_API_KEY') ?: '';
    $sender = 'hello@woafmeow.com';
    ...
    if (!function_exists('curl_init')) return ['status' => 'failed', 'detail' => 'The PHP cURL extension is unavailable.'];
    $guidePath = __DIR__ . '/WoafMeow_Senior_Dog_Care_Field_Guide.pdf';
    if (!is_readable($guidePath)) return ['status' => 'failed', 'detail' => 'The guide PDF is missing from the mail service.'];
    $guideContents = file_get_contents($guidePath);
    if ($guideContents === false) return ['status' => 'failed', 'detail' => 'The guide PDF could not be read.'];
    $guideUrl = 'https://labian1.github.io/assets/WoafMeow_Senior_Dog_Care_Field_Guide.pdf';
    $subject = 'Your 2026 Senior Dog Care Field Guide | WoafMeow';
    $updatesCopy = $marketingConsent
        ? '<p style="margin:0 0 18px">You also asked for occasional senior-dog care and WoafyPet updates. We will keep them useful and infrequent.</p>'
        : '<p style="margin:0 0 18px">This guide email does not add you to marketing updates.</p>';
    $payload = [
        'sender' => ['email' => $sender, 'name' => 'WoafMeow'],
        'to' => [['email' => $email]],
        'subject' => $subject,
        'htmlContent' => '<div style="font-family:Arial,sans-serif;max-width:640px;margin:0 auto;color:#332923;line-height:1.6"><p style="margin:0 0 18px">Hello,</p><h1 style="margin:0 0 18px;color:#1f4a3c;font-family:Georgia,serif;font-size:32px">Your Senior Dog Care Field Guide is attached.</h1><p style="margin:0 0 18px">WoafMeow helps dog parents notice quiet health changes earlier, organize what they are seeing, and prepare a clearer conversation with the veterinarian who knows their dog.</p><p style="margin:0 0 18px">You are joining a caring community of more than 10,000 pet owners who want practical guidance without judgment or guesswork.</p><p style="margin:0 0 18px">The attached 70-page field guide shows you how to build a useful baseline, track movement, sleep, appetite, water, bathroom habits and daily function, and recognize signs that deserve faster veterinary care.</p><p style="margin:0 0 18px"><a href="' . $guideUrl . '" style="display:inline-block;padding:13px 18px;background:#b8492f;color:#fff;text-decoration:none;border-radius:6px;font-weight:700">Open the guide online</a></p><p style="margin:0 0 18px">If your senior dog is becoming stiffer, less mobile, or experiencing weight changes alongside a chronic condition, you can also <a href="https://labian1.github.io/smart-bed/" style="color:#a6422b;font-weight:700">explore the WoafyPet Bed + Smart Base</a> for joint-focused comfort and passive pattern tracking. It does not diagnose or treat disease; your veterinary team remains the right place for medical decisions.</p>' . $updatesCopy . '<p style="margin:24px 0 0">With care,<br><strong>The WoafMeow team</strong><br><a href="mailto:hello@woafmeow.com" style="color:#a6422b">hello@woafmeow.com</a></p></div>',
        'attachment' => [[
            'content' => base64_encode($guideContents),
            'name' => 'WoafMeow_Senior_Dog_Care_Field_Guide.pdf',
        ]],
    ];
    $ch = curl_init('https://api.brevo.com/v3/smtp/email');
    curl_setopt_array($ch, [
    ...
    return ['status' => 'sent', 'detail' => 'Brevo accepted the Senior Dog Care Field Guide email.', 'subject' => $subject, 'sender' => $sender];
}

function saveSubmission(string $type, array $record, bool $sendOwnerNotification = true, bool $syncContact = true): void {
    ...
    if ($syncContact) syncBrevo($record, 'woafmeow.com/' . $type);
...
// Website forms are stored. Guide recipients join the Brevo update list only
// when they separately choose optional marketing updates.
$formRoutes = [
...
if (isset($formRoutes[$path]) && $method === 'POST') {
    ...
    if ($path === '/api/newsletter') {
        $guideConsent = filter_var($input['guideConsent'] ?? false, FILTER_VALIDATE_BOOLEAN);
        $marketingConsent = filter_var($input['marketingConsent'] ?? false, FILTER_VALIDATE_BOOLEAN);
        if (!$guideConsent) respond(422, ['error' => 'Choose guide delivery before submitting.']);
        $record['guideConsent'] = $guideConsent ? 'yes' : 'no';
        $record['marketingConsent'] = $marketingConsent ? 'yes' : 'no';
        saveSubmission($formRoutes[$path], $record, false, $marketingConsent);
        $delivery = sendGuideEmail($email, $marketingConsent);
        $log = readRows('notification-log');
        ...
        if (count($log) > 5000) $log = array_slice($log, -5000);
        saveRows('notification-log', $log);
        if ($delivery['status'] !== 'sent') respond(503, ['delivery' => 'fallback', 'guideUrl' => 'https://labian1.github.io/assets/WoafMeow_Senior_Dog_Care_Field_Guide.pdf', 'error' => 'The email was not sent. Download the guide directly instead.']);
        respond(201, ['delivery' => 'sent', 'sender' => $delivery['sender'], 'subject' => $delivery['subject'], 'message' => 'Your guide was sent from hello@woafmeow.com. Please check your inbox and spam folder.']);
    }
    saveSubmission($formRoutes[$path], $record);
